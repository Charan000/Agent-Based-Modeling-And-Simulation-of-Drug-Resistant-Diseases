 ...
 * <pre>
 * {@code
 * <method>:<definition> repast.simphony.action.display
 * }
 * </pre>
 * 
 * <P>
...
public class AgentFactory {
	...
	public AgentFactory(String agentDefinition) throws AgentCreationException {
		...
		int seed = 123456789;
		try {
			seed = Integer.parseInt(ContextManager.getParameter("randomSeed").toString());
	...
	private void createRandomAgents(boolean dummy) throws AgentCreationException {
		...
		while (agentsCreated < numAgents) {
			...
			while (i.hasNext() && agentsCreated < numAgents) {
				Building b = i.next();
				Building off = ContextManager.buildingContext.getRandomObject();// Find a building
				DefaultAgent a = new DefaultAgent(); // Create a new agent
				a.setHome(b); // Tell the agent where it lives
				a.setOffice(off);
				
				double typeChoice = generator.nextDouble();
				int type = -1;
				if(typeChoice <= 0.36)
					type = 1; // Classified as Student
				else if (typeChoice > 0.36 && typeChoice <= 0.72 )
					type = 2; // Classified as Employee
				else if (typeChoice > 0.72 && typeChoice <= 0.90 )
					type = 3; // Classified as Courier
				else
					type = 4; // Classified as Stay-At-Home
				a.setType(type);
				
				DefaultAgentState tempState = new DefaultAgentState(DefaultAgentState.State.U) ;
				
				double stateChoice = generator.nextDouble();
				if(stateChoice <= 0.7)
					tempState.setState(DefaultAgentState.State.U); // Classified as Unaffected
				else if (stateChoice > 0.7 && stateChoice <= 0.90 )
					tempState.setState(DefaultAgentState.State.LS); // Classified as Latent Susceptible Infection
				else
					tempState.setState(DefaultAgentState.State.AS); // Classified as Active Susceptible Infection
				
				a.setDisease(tempState);
				//LOGGER.info("Intializing States");
				DefaultAgent.typeCount.put(tempState.getCurrentState(), DefaultAgent.typeCount.get(tempState.getCurrentState())+1);
